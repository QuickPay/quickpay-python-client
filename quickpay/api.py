...
class QPApi(object):
  ...
  def __init__(self, secret=None, base_url=None, 
                api_version=None, timeout=305):
  ...
  def perform(self, method, path, **kwargs):
    ...
    if self.secret:
      headers["Authorization"] = "Basic {0}".format(_base64_encode(self.secret))
        
    if method in ['put', 'post', 'patch']:
      headers['content-type'] = 'application/json'
      response = self.fulfill(method, url, data=json.dumps(kwargs), 
                              headers=headers, timeout=self.timeout)
    else:
      response = self.fulfill(method, url, params=kwargs, 
                              headers=headers, timeout=self.timeout)
